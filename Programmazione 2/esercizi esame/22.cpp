...
template <class H> class Albero
{
	...
	//calcolo delle distanze dalla radice, se corrisponde a liv lo stampiamo
	void distanze(int m,int liv,ofstream & outfile,Nodo<H>* x)
	{
        ...
        if(x!=NULL)
		{
            ...
            distanze(m+1,liv,outfile,x->sx); //m rappresenta distanza tra x e x->sx
            if(m==liv){
                outfile<<x->val<<" ";
            }
            distanze(m+1,liv,outfile,x->dx); //m rappresenta distanza tra x e x->dx
        }
    }
	
	void print(ofstream & outfile,int liv)
	{
		distanze(0,liv,outfile,radice); //distanza parte da 0, liv passato in input ,si parte dalla radice ovviamente
		outfile<<endl;
	}
...
template <class H> void Albero<H>::print(int tipo_ord,ofstream & f)
{
	if(tipo_ord==0)
		pre_order(f,radice);
	if(tipo_ord==1)
